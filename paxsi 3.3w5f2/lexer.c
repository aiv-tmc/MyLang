    ...
    while (lexer->position < lexer->length) {
        ...
        if (lexer->input[lexer->position] == '\\') {
            ...
            while (depth > 0 && lexer->position < lexer->length) {
                if (lexer->input[lexer->position] == '\\') { SHIFT(lexer, 2); }
                else {
                    if (lexer->input[lexer->position] == '{') depth++;
                    else if (lexer->input[lexer->position] == '}') depth--;
                    if (depth > 0) SHIFT(lexer, 1); 
                }
            }

            if (depth > 0) {
                add_error(lexer, "Unclosed brace in string interpolation");
                free(buffer);
                return;
            }

            int expr_len = lexer->position - start_pos;
            char* expr_str = strndup(lexer->input + start_pos, expr_len);
            
            Lexer* sub_lexer = init_lexer(expr_str);
            sub_lexer->line = start_line;
            sub_lexer->column = start_col + 1;

            tokenize(sub_lexer);

            for (int i = 0; i < sub_lexer->token_count; i++) {
                if (sub_lexer->tokens[i].type == TOKEN_EOF) continue;
                
                Token t = sub_lexer->tokens[i];
                t.value = strdup(t.value);
                t.line = sub_lexer->tokens[i].line;
                t.column = sub_lexer->tokens[i].column;
