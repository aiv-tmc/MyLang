...
typedef enum {
    ...
    TOKEN_IF, TOKEN_ELIF, TOKEN_ELSE, TOKEN_DO,
    TOKEN_FUNCTION, TOKEN_START_FUNCTION, TOKEN_FUNCTION_LAMDA, TOKEN_RETURN,
    TOKEN_SWITCH, TOKEN_CASE, TOKEN_DEFAULT, TOKEN_BREAK, TOKEN_CONTINUE, TOKEN_QUIT,
    ...
    TOKEN_COMPILE, TOKEN_OUTSIDE_COMPILE, TOKEN_OUTSIDE_CODE
} TokenType;
...
static const KeywordEntry keywords[] = {
    ...
    {"char", TOKEN_TYPE}, {"bool", TOKEN_TYPE}, {"compile", TOKEN_COMPILE}, 
    {NULL, TOKEN_ERROR}
};
...
static void skip_balanced_block(Lexer* lexer);
static void parse_compile_statement(Lexer* lexer);
...
// Функция для пропуска сбалансированного блока {}
static void skip_balanced_block(Lexer* lexer) {
    int depth = 1;  // начинаем с 1, потому что мы уже на '{'
...
static void parse_compile_statement(Lexer* lexer) {
    ...
    // Ожидаем '('
    if (lexer->position >= lexer->length || lexer->input[lexer->position] != '(') {
        add_token(lexer, TOKEN_ERROR, "Expected '(' after 'compile'", 25);
        return;
    }
    ...
    // Разбираем параметр (идентификатор или строку)
    if (lexer->position < lexer->length) {
        char c = lexer->input[lexer->position];
        int start = lexer->position;
        if (c == '"' || c == '\'') {
            // Строковый литерал
            parse_string(lexer, c);
            // Меняем тип последнего токена на TOKEN_OUTSIDE_COMPILE
            if (lexer->token_count > 0) {
                lexer->tokens[lexer->token_count - 1].type = TOKEN_OUTSIDE_COMPILE;
            }
        }
            ...
            add_token(lexer, TOKEN_OUTSIDE_COMPILE, lexer->input + start, len);
        }
        else {
            add_token(lexer, TOKEN_ERROR, "Expected string or identifier", 28);
        }
    }
    ...
    // Ожидаем ')'
    if (lexer->position >= lexer->length || lexer->input[lexer->position] != ')') {
        add_token(lexer, TOKEN_ERROR, "Expected ')'", 12);
        return;
    }
    ...
    // Запоминаем начало блока (включая '{')
    int start_block = lexer->position;
    // Пропускаем блок
    skip_balanced_block(lexer);
    int end_block = lexer->position;
    int length_block = end_block - start_block;
    add_token(lexer, TOKEN_OUTSIDE_CODE, lexer->input + start_block, length_block);
}
...
void parse_operator(Lexer* lexer) {
    ...
    switch (character) {
        ...
        case ':':
            ...
            else add_token(lexer, TOKEN_COLON, ":", 1);
            break;
        case '.':
            if (next == '.' && double_next == '.') {
                add_token(lexer, TOKEN_ELLIPSIS, "...", 3);
                lexer->position += 3; lexer->column += 3;
                return;
            }
            else add_token(lexer, TOKEN_ERROR, ".", 1);
            break;
    ...
    while (lexer->position < lexer->length) {
        ...
        if (isalpha(character) || character == '_' || character == '$') {
            ...
            if (type == TOKEN_IDENTIFIER) {
                if (lexer->input[start] == '_') type = TOKEN_FUNCTION;
                else if (lexer->input[start] == '~') type = TOKEN_START_FUNCTION;
                else if (lexer->input[start] == '.') type = TOKEN_FUNCTION_LAMDA;
            }

            add_token(lexer, type, lexer->input + start, len);

            // Если это ключевое слово compile, обрабатываем конструкцию
            if (type == TOKEN_COMPILE) {
                parse_compile_statement(lexer);
            }
            continue;
        }
