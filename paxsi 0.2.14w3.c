...
void parse_number(Lexer* lexer) {
    ...
    // Check for numeral system prefixes (0x, 0o, 0b, 0q)
    if (lexer->position < lexer->length - 1 &&
...
bool is_valid_type(const char* type) {
    const char* valid_types[] = { 
        "int", "real", "char", "bool", "float", "double", 
        "long", "short", "void", "size_t", "ssize_t"
    };
...
static void parse_variable_declaration(Lexer* lexer) {
    ...
    // Check for pointer or reference symbols
    if (lexer->input[lexer->position] == '@') {
