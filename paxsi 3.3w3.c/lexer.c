...
void skip_whitespace(Lexer* lexer) {
    while (lexer->position < lexer->length) {
        if (lexer->input[lexer->position] == ' ' || lexer->input[lexer->position] == '\t') SHIFT(1);
    ...
    while (lexer->position < lexer->length) {
        ...
        switch (lexer->input[lexer->position]) {
            ...
            case 's':
                if (strncmp(lexer->input + lexer->position, "size", 4) == 0) {
                    ...
                    add_token(lexer, TOKEN_SIZE, "syscall", 7);
                    SHIFT(lexer, 7);
                } else goto identifier;
                break;        
        ...
        case '-':
            if (NEXT(lexer, 1) == '-') {
                ...
                add_token(lexer, TOKEN_RARROW, "->", 2);
                SHIFT(lexer, 2);
            } else {
        ...
        case '>':
            if (NEXT(lexer, 1) == '>' && NEXT(lexer, 2) == '>' && NEXT(lexer, 3) == '>') {
                add_token(lexer, TOKEN_ROR, ">>>>", 4);
                SHIFT(lexer, 4);
            } else if (NEXT(lexer, 1) == '>' && NEXT(lexer, 2) == '>') {
        ...
        case '<':
            if (NEXT(lexer, 1) == '<' && NEXT(lexer, 2) == '<' && NEXT(lexer, 3) == '<') {
                ...
                add_token(lexer, TOKEN_LARROW, "<-", 2);
                SHIFT(lexer, 2);
            } else {
        ...
        case '%':
            ...
            if (strncmp(lexer->input + lexer->position, "inclib", 6) == 0) {
                ...
                int length = lexer->position - start;
                add_token(lexer, TOKEN_PREPROC_VAR, lexer->input + start, length);
            }
            break;
