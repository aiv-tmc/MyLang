...
typedef enum {
    ...
    */
    TOKEN_MODIFIER,      // const, unsigned и др.
...
bool is_valid_modifier(const char* modifier) {
    const char* valid_modifiers[] = { "const", "unsigned", "extern", "static", "protected", "dynam", "regis", "local", "global" };
    for (int i = 0; i < 9; i++) {
...
bool is_valid_type(const char* type) {
    const char* valid_types[] = { "int", "real", "char", "bool", "func" };
    for (int i = 0; i < 5; i++) {
...
void tokenize(Lexer* lexer) {
    while (lexer->position < lexer->length) {
        ...
        case '_':
            add_token(lexer, TOKEN_UNDERLINING, "_", 1);
            lexer->position++;
            lexer->column++;
            break;
