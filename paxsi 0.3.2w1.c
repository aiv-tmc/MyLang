...
const char* token_names[] = {
    ...
    [TOKEN_PREPROC_ARCH]        =   "<TOKEN_PREPROC_ARCH>",
...
void free_lexer(Lexer* lexer) {
    for (int i = 0; i < lexer->token_count; i++) free(lexer->tokens[i].value);
    free(lexer->tokens);
    free(lexer);
}
...
static void parse_variable_access(Lexer* lexer) {
    ...
    } else add_error(lexer, "Expected identifier after '$' in variable access");
}
...
static void parse_variable_declaration(Lexer* lexer) {
    ...
    while (lexer->position < lexer->length && lexer->input[lexer->position] == '[') {
        ...
        while (lexer->position < lexer->length && lexer->input[lexer->position] != ']') {
            ...
            } else if (lexer->input[lexer->position] == '$') parse_variable_access(lexer);
            else {
    ...
    if (lexer->position < lexer->length && lexer->input[lexer->position] == '=') {
        ...
        if (lexer->position < lexer->length && lexer->input[lexer->position] == '{') {
            ...
            while (lexer->position < lexer->length) {
                ...
                if (lexer->input[lexer->position] == '\'') parse_char(lexer);
                else if (lexer->input[lexer->position] == '"') parse_string(lexer);
                else if (lexer->input[lexer->position] == '$') parse_variable_access(lexer);
                else if (isdigit(lexer->input[lexer->position]) ||
                    lexer->input[lexer->position] == '-' ||
                    lexer->input[lexer->position] == '+') {
...
void parse_macro(Lexer* lexer) {
    ...
    if (strncmp(lexer->input + lexer->position, "inclib", 6) == 0) {
    ...
    } else if (strncmp(lexer->input + lexer->position, "arch", 4) == 0) {
        lexer->position += 4;
        lexer->column += 4;
        ...
        if (lexer->input[lexer->position] != '[') {
            add_error(lexer, "Expected '[' after #arch");
            return;
        }
        ...
        if (lexer->position >= lexer->length) {
            add_error(lexer, "Unclosed #arch");
            return;
        }

        int length = lexer->position - start;
        add_token(lexer, TOKEN_PREPROC_ARCH, lexer->input + start, length);
...
void tokenize(Lexer* lexer) {
    while (lexer->position < lexer->length) {
        ...
        if (strncmp(lexer->input + lexer->position, "++", 2) == 0) {
        ...
        switch (character) {
            case '$': {
                ...
                if (first_char == '[') is_declaration = true;
                ...
                if (is_declaration) parse_variable_declaration(lexer);
                else parse_variable_access(lexer);
                break;
            }
            ...
            case '.':
                if (lexer->position < lexer->length - 2 &&
                    ...
                    lexer->position += 3;
                    lexer->column += 3;
                } else if (isdigit(next)) parse_number(lexer);
                else {
