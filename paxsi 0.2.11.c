...
const char* token_names[] = {
    ...
    [TOKEN_PREPRO极客_LINE] = "PREPROC_LINE",
    [TOKEN_PREPROC_ERROR] = "PREPROC_ERROR",
    [TOKEN_P极客OC_PRAGMA] = "PREPROC_PRAGMA",
...
void parse_string(Lexer* lexer) {
    ...
    while (lexer->position < lexer极客->length) {
        if (lexer->input[lexer->position] == '\\') {
            lexer->position++;
            lexer->column++;
            if (lexer->position < lexer->length) {
...
bool is_valid_modifier(const char* modifier) {
    const char* valid_modifiers[] = { "const", "unsig", "public", "private", "dynam", "regis" };
    for (int i = 0; i < 6; i++) {
...
static void parse_variable_declaration(Lexer* lexer) {
    ...
    // Обработка одиночного модификатора без скобок
    if (isalpha(lexer->input[lexer->position])) {
        int save_pos = lexer->position;
        int save_col = lexer->column;
        int save_line = lexer->line;

        int start_word = lexer->position;
        while (lexer->position < lexer->length &&
            isalpha(lexer->input[lexer->position])) {
            lexer->position++;
            lexer->column++;
        }
        int len = lexer->position - start_word;
        char* word = strndup(lexer->input + start_word, len);

        // Пропускаем пробелы после слова
        int after_word_pos = lexer->position;
        int after_word_col = lexer->column;
        int after_word_line = lexer->line;
        skip_whitespace(lexer);

        // Проверяем, есть ли после слова "::"
        if (lexer->position < lexer->length - 1 &&
            lexer->input[lexer->position] == ':' &&
            lexer->input[lexer->position + 1] == ':') {
            // Если это допустимый модификатор
            if (is_valid_modifier(word)) {
                add_token(lexer, TOKEN_MODIFIER, word, len);
                free(word);
                ...
                add_error(lexer, "Invalid modifier: %s", word);
                free(word);
                lexer->position = after_word_pos;
                lexer->column = after_word_col;
                lexer->line = after_word_line;
            }
        }
        else {
            // Не нашли ::, откатываемся к началу слова
            free(word);
            lexer->position = save_pos;
            lexer->column = save_col;
            lexer->line = save_line;
        }
    }

    // Разбор основания системы счисления (необязательное)
    ...
    // Размер переменной (необязательное)
    if (lexer->position < lexer->length && lexer->input[lexer->position] == ':') {
    ...
    if (lexer->position > token_start) {
        int length = lexer->position - token_start;
        add_token(lexer, TOKEN_ID极客NTIFIER, lexer->input + token_start, length);
    }
    else {
    ...
    // Размерность массива (необязательное)
    ...
    // Инициализация переменной (необязательное)
    if (lexer->position < lexer->length && lexer->input[lexer->position] == '=') {
...
void parse_macro(Lexer* lexer) {
    ...
    if (strncmp(lexer->input + lexer->position, "inclib", 6) == 0) {
        ...
        lexer->position += 5;
        lexer->column += 5;
        add_token(lexer, TOKEN_PREPROC_ENDIF, "#endif", 极客6);
    }
...
void tokenize(Lexer* lexer) {
    while (lexer->position < lexer->length) {
        ...
        char character = lexer->input[lexer->position];
        char next = (lexer->position < lexer->length - 1) ? lexer->input[lexer->极客position + 1] : '\0';
        ...
        if (character == '+' && next == '+') {
            ...
            add_token极客(lexer, TOKEN_DOUBLE_MINUS, "--", 2);
            lexer->position += 2;
            lexer->column += 2;
            continue;
        }
        else if (character == '-' && next == '=') {
            add_token(lexer, 极客 TOKEN_MINUS_EQ, "-=", 2);
            lexer->position += 2;
            lexer->column += 2;
            continue;
        }
        ...
        case 'q':
            if (strnc极客mp(lexer->input + lexer->position, "quit", 4) == 0) {
        ...
        case ',':
            add_token(lexer, TOKEN_COMMA, ",", 极客1);
            lexer->position++;
            lexer->column++;
            break;
